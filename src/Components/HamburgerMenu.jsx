...
const HamburgerMenu = ({ className = "" }) => {
  ...
  return (
    <div className={`z-30 h-8 top-5 right-16 absolute md:hidden ${className}`}>
      ...
      <label
        htmlFor="menu-toggle"
        className="h-8 w-8 flex relative cursor-pointer"
      >
        {/* first line */}
        <div
      ...
      {isOpen && (
        <div className="absolute top-16 -right-[10px] w-[85vw] max-w-sm  rounded-xl bg-black/10 dark:bg-white/10 backdrop-blur-xl border flex flex-col gap-2 border-white/20 text-2xl text-black dark:text-white p-6 z-20 shadow-lg  max-h-[calc(100vh-80px)] overflow-y-auto ">
          ...
          <a
            ...
            target="_blank"
            className="flex gap-2 border-gray-00 border-2 rounded-md p-2  w-70 h-12 items-center md:border-none md:h-10 md:items-center md:justify-center md:w-10 hover:border-black hover:bg-black hover:text-white hover:dark:white hover:dark:text-black hover:dark:bg-white  border-gray-800  transition-all duration-200 ease-in-out dark:text-white dark:border-white hover:dark:border-white"
          >
            {" "}
            <IoIosMail />
          ...
          <div className="flex gap-2 border-gray-00 border-2 dark:bg-gray-800 rounded-md p-2  w-70 h-12 items-center md:border-none md:h-10 md:items-center md:justify-center md:w-10   border-gray-800   transition-all duration-200 ease-in-out dark:text-white dark:border-gray-800 hover:dark:border-white ">
            {" "}
            <DarkMode /> <p className="text-[16px]">Light / Dark Mode</p>{" "}
          </div>
